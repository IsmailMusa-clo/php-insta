...
class User {
  ...
  public function isClosed() {
    if (!isset($this->user['username']) || empty($this->user['username'])) {
        return false; // أو يمكنك تسجيل خطأ أو اتخاذ إجراء آخر
    }
    ...
    if (!$query) {
        error_log("خطأ في الاستعلام: " . mysqli_error($this->con));
        return false; // أو يمكنك إرجاع خطأ مناسب
    }

    $row = mysqli_fetch_array($query);

    // التحقق من وجود البيانات
    if (!$row || !isset($row['user_closed'])) {
        return false; // أو يمكنك تعديل الإجراء هنا
    }
